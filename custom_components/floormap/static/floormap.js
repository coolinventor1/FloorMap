...
function roomBoundsFromPoints(points) {
...
function clampRoomPoint(point) {
  return {
    x: clampCoordinate2(point.x),
    y: clampCoordinate2(point.y)
  };
}
...
function defaultRoomLabelPosition(room) {
  const bounds = roomBoundsFromPoints(room.points);
  return {
    x: clampCoordinate2(bounds.x + 0.01),
    y: clampCoordinate2(bounds.y + 0.01)
  };
}
function roomLabelStyle(room) {
  return `left:${room.label_x * 100}%; top:${room.label_y * 100}%;`;
}
function roomRelativePoints(room) {
  const bounds = roomBounds(room);
...
function roomPolygonPoints(room) {
  return roomRelativePoints(room).map((point) => `${point.x.toFixed(2)},${point.y.toFixed(2)}`).join(" ");
}
function roomClipPath(room) {
  return `polygon(${roomRelativePoints(room).map((point) => `${point.x.toFixed(2)}% ${point.y.toFixed(2)}%`).join(", ")})`;
}
function roomContainsPoint(room, x2, y3) {
  let inside = false;
  for (let index = 0, previous = room.points.length - 1; index < room.points.length; previous = index++) {
    ...
    const intersects = currentPoint.y > y3 !== previousPoint.y > y3 && x2 < (previousPoint.x - currentPoint.x) * (y3 - currentPoint.y) / (previousPoint.y - currentPoint.y || Number.EPSILON) + currentPoint.x;
    if (intersects) {
      inside = !inside;
    }
  }
...
var DEFAULT_ROOM_NAME = "New room";
var baseStyles = i`
  :host {
    display: block;
...
function roomPointHandleStyle(point) {
...
var FloorMapCard = class extends FloorMapBaseElement {
  ...
  _renderCardRoomLabel(room) {
    return b2`<div class="room-label" style=${roomLabelStyle(room)}>${room.name}</div>`;
  }
...
var FloorMapPanel = class extends FloorMapBaseElement {
  constructor() {
    ...
    this._onRoomNameInput = (event) => {
      this._roomName = event.currentTarget.value;
    };
    this._startRoomDraw = () => {
      this._pendingEntityId = null;
    ...
    this._cancelRoomDraw = () => {
      this._pendingRoomName = null;
      this._pendingRoomPoints = [];
    };
    this._undoPendingRoomPoint = () => {
      this._pendingRoomPoints = this._pendingRoomPoints.slice(0, -1);
    };
    this._finishRoomDraw = () => {
        ...
        <aside class="sidebar">
          ...
          <section class="section">
            ...
            <div class="row-actions">
              ...
              ${this._pendingRoomName ? b2`<button @click=${this._undoPendingRoomPoint} ?disabled=${!this._pendingRoomPoints.length}>Undo point</button>` : A}
              ${this._pendingRoomName ? b2`<button @click=${this._finishRoomDraw} ?disabled=${this._pendingRoomPoints.length < MIN_ROOM_POINTS}>Done</button>` : A}
              ${this._pendingRoomName ? b2`<button @click=${this._cancelRoomDraw}>Cancel</button>` : A}
            </div>
            <div class="room-list">
              ${this._draftRooms.length ? this._draftRooms.map((room) => this._renderRoomRow(room)) : b2`<div class="muted">No rooms defined yet.</div>`}
            </div>
          </section>
            ...
            >
              <div class="map-stage" style=${this._imageStageStyle()}>
                ...
                <div class="rooms">
                  ...
                  ${this._pendingRoomPoints.length >= MIN_ROOM_POINTS ? this._renderDraftRoom() : A}
                </div>
                <div class="room-labels">
    ...
    return b2`
      <div class="room-row">
        ...
        <input
          type="text"
          .value=${room.name}
          @input=${(event) => this._updateRoomName(room.id, event)}
        />
        <div class="row-actions">
  ...
  _renderEditorRoom(room, preview = false) {
    const activeLights = activeLightPlacementsForRoom(
      room,
      this._draftPlacements,
      (entityId) => this.hass?.states[entityId],
  ...
  _updateRoomName(roomId, event) {
    const target = event.currentTarget;
    const nextName = target.value.trim() || DEFAULT_ROOM_NAME;
    this._draftRooms = this._draftRooms.map(
      (room) => room.id === roomId ? {
        ...room,
        name: nextName
      } : room
  ...
  _onEditorPointerDown(event) {
    const target = event.target;
    const marker = target.closest(".marker-chip");
    const roomPointHandle = target.closest(".room-point-handle");
    const roomLabel = target.closest(".room-label.is-editable");
    const surface = this._mapSurface();
    if (!surface) {
      return;
    }
  ...
  _onEditorPointerMove(event) {
    ...
    if (activeState.mode === "marker") {
      const point2 = this._normalizedPointFromClient(event.clientX, event.clientY);
      if (!point2) {
        return;
      }
      this._draftPlacements = this._draftPlacements.map(
        (placement) => placement.entity_id === activeState.entityId ? { ...placement, x: point2.x, y: point2.y } : placement
      );
      this._dirty = true;
      return;
    }
    ...
    if (activeState.mode === "room-point") {
      this._draftRooms = this._draftRooms.map(
        (room) => room.id === activeState.roomId ? {
          ...room,
          points: room.points.map(
            (roomPoint, index) => index === activeState.pointIndex ? clampRoomPoint(point) : roomPoint
          )
        } : room
      );
      this._dirty = true;
  ...
  _onEditorMapClick(event) {
    const target = event.target;
    if (target.closest(".marker-chip") || target.closest(".room-point-handle") || target.closest(".room-label.is-editable")) {
      return;
    }
    const point = this._normalizedPointFromClient(event.clientX, event.clientY);
    if (!point) {
